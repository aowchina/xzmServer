...
$db = JFactory::getDbo();
$id = $this->item->id;
//$appuid= $this->item->appuid;
//
//$group_sql = "select a.title,c.level from #__usergroups a left join #__user_usergroup_map b on b.group_id = a.id left join #__users c on b.user_id = c.id where c.id =$appuid";
//$db->setQuery($group_sql);
$group = $db->loadAssoc();
$sql = "select a.goodid, a.amount, a.money, b.name, c.shopname from #__order_goods as a join #__good as b join #__shop as c where a.id = '$id'
...
<form action="<?php echo JRoute::_('index.php?option=com_order&layout=edit&id='.(int)$this->item->id); ?>" 
