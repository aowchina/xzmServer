...
class UstyleModelCimg extends JModelAdmin
{
	protected $text_prefix = 'COM_USTYLE';
	public function getTable($type = 'Cimg', $prefix = 'UstyleTable', $config = array()){
		return JTable::getInstance($type, $prefix, $config);
	}
	public function getForm($data = array(), $loadData = true){
		$app = JFactory::getApplication();
		$form = $this->loadForm('com_ustyle.cimg', 'cimg',array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData(){
		$data = JFactory::getApplication()->getUserState('com_ustyle.edit.cimg.data', array());
		if (empty($data)){
			$data = $this->getItem();
		}
		return $data;
