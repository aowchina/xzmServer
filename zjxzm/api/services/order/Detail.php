...
//        $sql = "select img,name,oem from zj_goods where goodid = '".$order_item['goodid']."'";
        ...
        $imgs=json_decode($ginfo['img']);
        if($imgs){
            $img = $imgs[0];
            $order_list[$key]['img'] = $s_url.$img;
        }else{
            $order_list[$key]['img'] = " ";
        }

        $order_list[$key]['name'] = $ginfo['name'];
        $order_list[$key]['oem']= isset($order_item['oem'])?$ginfo['oem']:" ";
        $order_list[$key]['bname'] = $ginfo['bname'];
        $order_list[$key]['sname'] = $ginfo['sname'];
        $order_list[$key]['cname'] = $ginfo['cname'];
    }
}else{
    $order_list = array();
}
...
$r_data['money'] = $order_list['money'];  //现在直接取的是商品的价格(单价),待改
...
$r_data['total_money'] = $order_list['money']* $order_item['amount'];
