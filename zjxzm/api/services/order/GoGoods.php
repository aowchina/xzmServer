...
/**
 * 发货(卖家端)
 * * 接口参数: 8段 * sellerid * 订单号(orderid) * 快递单号 * 快递id
 * author zq
 * date 2017-6-19
 */
...
$count = dbCount('zj_order', $con, "orderid = '".$order_num."' and shopid = $result[shopid] and status = 1");
if($count != 1){
    forExit($lock_array, $con);
...
if(!dbUpdate($data, "zj_order", $con, "orderid = '$order_num'")){
    forExit($lock_array, $con);
