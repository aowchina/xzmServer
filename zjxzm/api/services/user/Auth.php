...
// file_put_contents($public_url.'file',json_encode($_FILES));
if($imgamount > 0){
    //文件上传
    for($i = 0; $i < $imgamount; $i++){
        ...
        if($i == 0)
        {
            ...
            $new_filename = 'userid_'.$userid.".".$img_suffix;
        }
        else
        {
...
//处理地址
$address = areaname($pid,$con)." ".areaname($cid,$con)." ".areaname($qid,$con);
$data['address'] = $address;

$count = dbCount('zj_sellercert',$con,"sellerid = $userid");
if($count >= 1)
{
    if(!dbUpdate($data,'zj_sellercert', $con, "sellerid = $userid")){
        forExit($lock_array, $con);
...
else
{
    ...
    if(!dbAdd($data, 'zj_sellercert', $con)){
        forExit($lock_array, $con);
        toExit(302, $return_list);
    }
}
  ...
  $r_data['is_rz']= 0;//审核中
    if(!dbUpdate($r_data,'zj_seller', $con, "sellerid = '$userid'")){
        forExit($lock_array, $con);
        toExit(302, $return_list);
    }

//根据id显示名称
function areaname($id,$con){
    $sql = "select areaname from zj_area where id = ".$id;
    $areaname = dbLoad(dbQuery($sql,$con),true);
