...
try{
   //给卖家发送通知
    $sql = "select deviceid from zj_user_login where is_app=0";
    ...
    if(count($result) > 0){
        foreach ($result as $key => $value) {
            //获取极光id
            $jpushid = file_get_contents($j_path.'device/'.getSubPath($value['deviceid'], 4, true).'jpush');
            echo $jpushid;echo '<br>';echo '2093487203948230948230948209348203948209348';
            if($jpushid){
                $jp = new Jpush();
                $jp->push($jpushid,'您收到一条新的求购信息');
            }
        }
