<?php
// $j_path = "/var/www/html/zjxzm/data/";      //Josn文件保存路径
$j_path ="/data/pubout/minfo/zjxzm/data/";
$s_url = "http://".$_SERVER['HTTP_HOST']."/";     //后台URL
$s_path = '/data/pubout/minfo/zjxzm/';           //后台URL
$t_path = '/data/pubout/minfo/zjxzm/sellerdata/';
$appid_name_array = array("7000000008"=>"zjxzm");
$appid_key = array("7000000008" => "MINFOCARREPAIRPURCHASEGUOZHANGTONGPEI");
...
function pswData($data_str){
    ...
    for($i = 0; $i < $data_array['cnt']; $i++){
        ...
        $command = "";
        $command = "../Alg 1 '".$sub_data_str."' ".md5("MINFOCARREPAIRPURCHASEGUOZHANGTONGPEI");
 ...
 * @author wangrui
 */
// function toExit($errorcode, $return_list=array(), $needpsw = true){
//     //$resturn_list必须为数组
//     if(!is_array($return_list)){
//         $return_list=array();
//         echo json_encode($return_list['errorcode']=101);
//         exit;
//     }
...
function sendMsg($tel,$code){
    ...
    $user = "wangrui"; //短信平台帐号
    $pass = md5("111111"); //短信平台密码
    $content="【浙江心之盟】验证码为:".$code."，有效期为30秒";//要发送的短信内容
    $sendurl = $smsapi."sms?u=".$user."&p=".$pass."&m=".$tel."&c=".urlencode($content);
    $result =file_get_contents($sendurl);
    if($result == 0){
        return true;
