...
$sql = "select appuid from zj_appuser where tel = '$tel'";
$result = dbLoad(dbQuery($sql, $con), true);
$userid = $result['appuid'];
...
//注册到环信
$hx = new Easemob($hx);
$create_result = $hx->createUser('buy'.$userid, $tel, $name);

if(!$create_result){
    if(dbDel('zj_appuser',$con,'appuid='.$userid)){
        forExit($lock_array, $con);
        toExit(63, $return_list);//环信注册失败
    }else{
        forExit($lock_array, $con);
        toExit(631, $return_list);//环信注册失败,用户删除失败
    } 
}
