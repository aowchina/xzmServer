...
*/
//openid是否存在
$condition = "typeid = $type and is_app = 1 and openid = '" . mysql_real_escape_string($openid, $con) . "'";
...
if ($count > 0) {
    ...
    /*********返回参数修改**********/
    $sql = "select name,picture from zj_appuser where appuid=".$userid;
    ...
    $data['picture'] = empty($userData2['picture']) ? '' : $s_url.$userData2['picture'];
    /*********返回参数修改**********/
    ...
    //取出即将插入的id
    $sql = "SHOW TABLE STATUS like 'zj_appuser'";
    ...
    if (dbAdd($data, 'zj_appuser', $con)) {
        ...
        /***************注册环信开始********************/
        $sql = "select name from zj_appuser where appuid=".$userid;
        $name = dbLoad(dbQuery($sql,$con),true)['name'];

        //注册到环信
        $hx = new Easemob($hx);
        $create_result = $hx->createUser('buy'.$userid, $openid, $name);
        ...
        $return['nickname'] = $userData['name'];
        $return['picture'] = $s_url.$userData['picture'];
        // /*********返回参数修改**********/
